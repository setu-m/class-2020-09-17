...
```{r Scene 1}
raw_data <- read_excel("raw_data/class_enrollment_summary_by_term_9-16-2020.xlsx", skip = 3) %>%
  select(-c(Grad, NonDegree, XReg, VUS, Employee, Withdraw, Total)) %>%
  janitor::clean_names()

en_fall_2020 <- raw_data %>%
  filter(course_title != "NA") %>%
  filter(u_grad >= 10) %>%
